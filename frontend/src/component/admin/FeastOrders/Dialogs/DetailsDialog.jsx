                            ...
                            >
                                <Box>
                                    <Table.Root variant="surface">
                                        ...
                                        <Table.Body>
                                            {selectedOrderDetails?.compulsoryItems?.map((item) => (
                                                <Table.Row key={item._id}>
                                                    <Table.Cell>{item.item.name}</Table.Cell>
                                                    <Table.Cell>{item.item.category}</Table.Cell>
                                                    {/* <Table.Cell>{item.quantity}</Table.Cell> */}
                                                    <Table.Cell>Rs. {item.item.pricePerPerson ? item.item.pricePerPerson : "Included in Base Plate"}</Table.Cell>
                                                </Table.Row>
                                            ))}
                                            {selectedOrderDetails?.additionalItems?.map((item) => (
                                                <Table.Row key={item._id}>
                                                    <Table.Cell>{item.item.name}</Table.Cell>
                                                    <Table.Cell>{item.item.category}</Table.Cell>
                                                    {/* <Table.Cell>{item.quantity}</Table.Cell> */}
                                            ...
                                            {selectedOrderDetails?.desserts?.map((item) => (
                                                <Table.Row key={item._id}>
                                                    <Table.Cell>{item.item.name}</Table.Cell>
                                                    <Table.Cell>{item.item.category}</Table.Cell>
                                                    {/* <Table.Cell>{item.quantity}</Table.Cell> */}
                                                    <Table.Cell>{item.item.pricePerPerson ? `Rs. ${item.item.pricePerPerson}` : "Included in Base Plate"}</Table.Cell>
                                                </Table.Row>
                                            ))}
                                        </Table.Body>
                                    </Table.Root>
                                    <Flex justify="end" mt="4">
                                        <Text weight="bold">
                                            Total: Rs. {selectedOrderDetails?.totalPrice}
                                        </Text>
                                    </Flex>
                                </Box>
                            </motion.div>
                                ...
                                <Box>
                                    <Flex direction="column" gap="3">
                                        ...
                                        <Flex gap="2">
                                            ...
                                            <Text>{selectedOrderDetails?.status}</Text>
                                        </Flex>
                                        <Flex gap="2">
                                         ...
                                         <Flex gap="2">
                                            <Text weight="bold">People Count:</Text>
                                            <Text>
                                                {selectedOrderDetails?.peopleCount}
                                            </Text>
                                        </Flex>
                                    </Flex>
                                </Box>
